...
def pca(X = np.array([]), no_dims = 50):
    ...
    N, D = X.shape
    S = np.matmul(X.transpose(), X) / N
    eig_value, eig_vector = np.linalg.eigh(S)
    eig_vector = eig_vector.transpose()

    for i in range(no_dims):
        idx = len(eig_value) - i - 1
        M = np.append(M, eig_vector[idx])
    M = M.reshape(no_dims, D).transpose()
...
def decompress(Y = np.array([]), M = np.array([])):
    ...
    N, _ = Y.shape
    D, _ = M.shape
    X_hat = np.matmul(Y, M.transpose()).reshape(N, D)
