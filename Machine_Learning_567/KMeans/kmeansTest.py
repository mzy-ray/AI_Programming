...
def transform_image(image, code_vectors):
    ...
    new_im = np.zeros(image.shape[0] * image.shape[1] * image.shape[2]).reshape(image.shape[0], image.shape[1], image.shape[2])
    for i in range(0, len(image)):
        for j in range(0, len(image[i])):
            min_dist = np.inf
            for cv in code_vectors:
                distance = np.sum(np.square(np.array(image[i][j]) - np.array(cv)))
